...
from blockchain_api import BlockchainDataAPI
# Import the enhanced fraud model
from fraud_model import load_blockchain_fraud_model
...
model = load_blockchain_fraud_model()

# Initialize blockchain API
blockchain_api = BlockchainDataAPI()

# Initialize the Dash app
app = dash.Dash(__name__, title="Live Blockchain Fraud Detection Dashboard")
...
app.layout = html.Div([
    ...
    dcc.Interval(
        ...
        interval=30*1000,  # update every 5 seconds
        n_intervals=0
    )
...
def update_data(n):
    ...
    # Fetch new transactions from blockchain API
    new_blockchain_txs = blockchain_api.get_latest_transactions(limit=5)
    
    # If we got new transactions, process them
    if new_blockchain_txs:
        new_transactions = []
        
        # Convert to DataFrame for batch processing
        txs_df = pd.DataFrame(new_blockchain_txs)
        
        # Use the model to analyze all transactions at once
        if not txs_df.empty:
            results = model.predict(txs_df)
            
            # Add results to each transaction
            for i, tx in enumerate(new_blockchain_txs):
                tx['Fraud_Score'] = float(results['ensemble_score'][i])
                tx['Is_Fraud'] = tx['Fraud_Score'] > 0.7
        ...
        for tx in new_transactions:
            if tx['Is_Fraud']:
                ...
                alert['Description'] = f"Suspicious transaction of {tx['Transaction_Amount']:.4f} BTC detected"
        ...
        # If no new transactions, still update the chart with zeros
        transaction_counts['timestamps'].append(current_time.strftime("%H:%M:%S"))
        transaction_counts['normal'].append(0)
        transaction_counts['fraud'].append(0)
...
def analyze_transaction(n_clicks, tx_id, sender_id, receiver_id, amount):
    ...
    try:
        ...
        explanation = model.explain_prediction(tx_df)
        fraud_score = explanation['score']
